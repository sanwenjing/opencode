...
description: "OpenClaw配置管理工具，提供模型配置、渠道配置、Gateway管理等常用命令"
version: "1.0.0"
...
管理远程OpenClaw实例的配置，包括：
- 模型配置（查看、设置默认模型、认证）
- 渠道配置（飞书DM策略、WebSocket模式）
- Gateway管理（启动、重启）
- 日志查看和健康检查
...
| 脚本名称 | 脚本路径 | 功能描述 | 调用方式 |
|---------|---------|---------|---------|
| main.py | scripts/main.py | 主执行脚本：OpenClaw配置管理 | `python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" <命令>` |
| requirements.txt | scripts/requirements.txt | Python依赖列表 | `pip install -r "/root/.config/opencode/skills/openclaw-config/scripts/requirements.txt"` |
...
```bash
pip install -r "/root/.config/opencode/skills/openclaw-config/scripts/requirements.txt"
```
...
```bash
# 查看模型列表
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" models list -n node-2224

# 查看模型状态
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" models status -n node-2224

# 设置默认模型
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" models set "NexaAI/Qwen3-0.6B-GGUF:Q4_0" -n node-2224

# 查看配置
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" config get "env.OPENAI_API_KEY" -n node-2224

# 设置配置
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" config set "env.OPENAI_API_KEY" "sk-xxx" -n node-2224

# 启用飞书WebSocket
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" feishu websocket -n node-2224

# 设置飞书DM策略
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" feishu dm anyone -n node-2224

# 重启Gateway
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" gateway restart -n node-2224

# 查看日志
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" logs -n node-2224

# 健康检查
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" doctor -n node-2224

# 查看状态
python3 "/root/.config/opencode/skills/openclaw-config/scripts/main.py" status -n node-2224
```

#### 参数说明

- `-n, --host`: 目标主机名称（默认: node-2224）
