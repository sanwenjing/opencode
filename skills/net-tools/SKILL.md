...
封装nmap网络扫描工具，提供简化的命令行接口，支持常用扫描场景：
- 单IP/域名快速扫描
- 端口扫描（指定端口、端口范围、常用端口）
- 主机发现（ARP扫描、Ping扫描）
- 服务版本检测
...
- macOS: `brew install nmap` 或已预装

### 基本用法

```bash
# 扫描单个主机（默认1000个常用端口）
python3 /root/.config/opencode/skills/net-tools/scripts/main.py scan 192.168.1.1

# 扫描指定端口
python3 /root/.config/opencode/skills/net-tools/scripts/main.py scan 192.168.1.1 -p 80,443

# 快速扫描（100个常用端口）
python3 /root/.config/opencode/skills/net-tools/scripts/main.py quick 192.168.1.1

# 主机发现（仅检测在线主机）
python3 /root/.config/opencode/skills/net-tools/scripts/main.py discover 192.168.1.0/24

# 服务版本检测
python3 /root/.config/opencode/skills/net-tools/scripts/main.py service 192.168.1.1

# 操作系统检测
python3 /root/.config/opencode/skills/net-tools/scripts/main.py os 192.168.1.1

# 完整扫描（包含版本和系统检测）
python3 /root/.config/opencode/skills/net-tools/scripts/main.py full 192.168.1.1

# 保存结果到文件
python3 /root/.config/opencode/skills/net-tools/scripts/main.py scan 192.168.1.1 -o result.txt
```
...
| 命令 | 说明 | 示例 |
|-----|------|------|
| scan | 基本端口扫描 | `scan 192.168.1.1` |
| quick | 快速扫描（100端口） | `quick 192.168.1.1` |
| discover | 主机发现 | `discover 192.168.1.0/24` |
| service | 服务版本检测 | `service 192.168.1.1` |
| os | 操作系统检测 | `os 192.168.1.1` |
| full | 完整扫描 | `full 192.168.1.1` |

### 参数说明

| 参数 | 说明 |
|-----|------|
| -p, --ports | 指定端口（逗号分隔或范围） |
| -o, --output | 保存结果到文件 |
| -v, --verbose | 显示详细输出 |
| -t, --timing | 扫描速度（0-5） |
